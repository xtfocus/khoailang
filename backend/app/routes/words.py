from fastapi import APIRouter, UploadFile, File, HTTPException, Depends, Request, Query, Body
    ...
    async with semaphore:
        try:
            response = await clients["openai"].responses.create(
                ...
                input=[
                    {"role": "system", "content": "You are a helpful assistant that validates words and phrases. Return only valid words or phrases."},
                    {"role": "user", "content": "\n".join(batch)}
                ],
                text={
                    "format": {
    ...
    async with semaphore:
        try:
            response = await clients["openai"].responses.create(
                model=configs["app_config"].OPENAI_MODEL,
                input=[
                    {"role": "system", "content": "Generate concise definitions for the following words or phrases. Return as a JSON array of objects with 'front' (word) and 'back' (definition) properties."},
                    {"role": "user", "content": "\n".join(batch)}
                ],
                text={
                    "format": {
...
async def generate_flashcards(words: List[str]):
    ...
    # Process batches with semaphore control
    results = await gather(*[generate_flashcards_batch(batch, semaphore) for batch in batches])
...
async def get_languages(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            "languages": [{"id": lang.id, "name": lang.name, "code": lang.code} for lang in languages]
        }
    except Exception as e:
